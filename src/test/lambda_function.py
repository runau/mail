...
def lambda_handler(event, context):
    ...
    #もしtextメッセージが用意されてなかったら、htmlを出力
    if body_text != "":
        body = body_text
    else:
    ...
    for define in defines:
        if "topicName" in define:
            topic_arn = f"{os.environ['TOPIC_ARN']}{define['topicName']}"
            subject = f'メール受信:{email_header["Subject"]}'
            send_mail_by_sns(topic_arn,mess,subject)
        ...
        if "queueName" in define:
            ...
            value = define
            value["message"] = mess
            response = send_sqs_message(define["queueName"],json.dumps(value))
